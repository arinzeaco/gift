...
class TransferRedeemUseCase @Inject constructor(private val transferRepository: TransferRepository) {
    ...
    suspend fun execute(request: TransferCommissionRequest): Result {
        return try {
            Result.Success(transferRepository.transferRedeem(request))

...
class ChargePayPalUseCase @Inject constructor(private val paymentTokenRepository: PaymentTokenRepository) {
    ...
    suspend fun execute(payLink: String): Result {
        return try {
            Result.Success(paymentTokenRepository.chargePayPal(payLink))

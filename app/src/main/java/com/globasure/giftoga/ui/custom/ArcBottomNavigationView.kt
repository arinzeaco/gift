...
/**
 * Created by longnguyen on 7:33 AM, 4/2/20.
 *
 */
class ArcBottomNavigationView : BottomNavigationView {
    ...
    private var buttonMargin = MIN_BUTTON_MARGIN.toPixel()
        set(value) {
            if (field != value) {
                field = max(value, MIN_BUTTON_MARGIN.toPixel())
                updatePoints(width)
            }
        }
    ...
    private var buttonIconSize: Float = buttonSize
        set(value) {
            field = min(buttonSize, value)
            button?.apply {
                iconSize = field.toInt()
            }
        }

    private var buttonStrokeWidth: Float = 0.0f
        set(value) {
            field = value
            button?.apply {
    ...
    private var buttonStrokeColor: Int = Color.TRANSPARENT
        set(value) {
            field = value
            button?.apply {
    ...
    private var buttonIconTint: Int = Color.TRANSPARENT
        set(value) {
            field = value
            button?.apply {
    ...
    private var buttonBackgroundTint: Int = Color.TRANSPARENT
    ...
    private var buttonRippleColor: Int = Color.TRANSPARENT
        set(value) {
            field = value
            button?.apply {
            ...
            value?.let {
                menuView?.apply {
                    for (index in 0 until childCount) {
                        ...
                        val smallItemText =
                            item.findViewById<View>(com.google.android.material.R.id.smallLabel)
                        if (smallItemText is TextView) {
                            smallItemText.typeface = value
                        }
                        val largeItemText =
                            item.findViewById<View>(com.google.android.material.R.id.largeLabel)
    ...
    private var state: State = currentState
        set(value) {
            field = value
            transitionTo(value)
        }
    ...
    private val visibleBoundPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        color = Color.WHITE //add default color
    }
    ...
    private var deselectOnButtonClick: Boolean = false
    ...
    constructor(
        ...
        attrs?.apply {
            ...
            buttonSize =
                ta.getDimension(R.styleable.ArcBottomNavigationView_ai_buttonSize, buttonSize)
            buttonMargin =
                ta.getDimension(R.styleable.ArcBottomNavigationView_ai_buttonMargin, buttonMargin)
            if (ta.hasValue(R.styleable.ArcBottomNavigationView_ai_buttonIcon)) {
                val iconResId =
                    ta.getResourceId(R.styleable.ArcBottomNavigationView_ai_buttonIcon, 0)
                if (iconResId > 0)
                    buttonIcon = VectorDrawableCompat.create(resources, iconResId, null)
            }
            buttonIconSize = ta.getDimension(
                R.styleable.ArcBottomNavigationView_ai_buttonIconSize,
                buttonIconSize
            )
            buttonStrokeWidth =
                ta.getDimension(
                    R.styleable.ArcBottomNavigationView_ai_buttonStrokeWidth,
                    buttonStrokeWidth
                )
            buttonStrokeColor = ta.getColor(
                R.styleable.ArcBottomNavigationView_ai_buttonStrokeColor,
                buttonStrokeColor
            )
            buttonBackgroundTint =
                ta.getColor(
                    R.styleable.ArcBottomNavigationView_ai_buttonBackgroundTint,
                    buttonBackgroundTint
                )
            buttonRippleColor = ta.getColor(
                R.styleable.ArcBottomNavigationView_ai_buttonRippleColor,
                buttonRippleColor
            )
            buttonIconTint =
                ta.getColor(R.styleable.ArcBottomNavigationView_ai_buttonIconTint, buttonIconTint)
            ...
            deselectOnButtonClick =
                ta.getBoolean(R.styleable.ArcBottomNavigationView_ai_selectableButton, false)
            showShadow = ta.getBoolean(R.styleable.ArcBottomNavigationView_ai_show_shadow, false)
            ta.recycle()
        }
        ...
        if (showShadow) visibleBoundPaint.setShadowLayer(10F, 5F, 5F, Color.argb(80, 0, 0, 0))
        ...
        button = ArcButton(context)
            .apply {
                ...
                buttonIcon?.let { icon = buttonIcon!! }
     ...
     */
    private fun shouldAnimate(point: PointF) =
        point.y > visibleBound.top && point.y < visibleBound.bottom
            ...
            .apply {
                ...
                addUpdateListener {
                    if (points.isNotEmpty()) {
                        ...
                        val factor =
                            if (state == State.FLAT) 1.0f - animatedValue else animatedValue
                        points.forEachIndexed { index, point ->
                            if (!isCorner(point))
                        ...
                        arcAnimationListener?.onArcAnimationUpdate(
                            animatedValue,
                            currentState,
                            state
                        )
